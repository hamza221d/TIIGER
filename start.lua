sudo_TIIGER = dofile("sudo.lua")
https = require("ssl.https")
JSON = dofile("./libs/JSON.lua")
local TIIGER_dev = io.open("TIIGER_online.lua")
if TIIGER_dev then
TIIGER_on = {string.match(TIIGER_dev:read('*all'), "^(.*)/(%d+)")}
local tsheke_file = io.open("sudo.lua", 'w')
tsheke_file:write("token = '" ..TIIGER_on[1].."'\n\nsudo_add = "..TIIGER_on[2].."" )
tsheke_file:close()
https.request("https://api.telegram.org/bot"..TIIGER_on[1].."/sendMessage?chat_id="..TIIGER_on[2].."&text=Bot_TIIGER_is_start_new")
os.execute('cd .. && rm -rf .telegram-cli')
os.execute('rm -rf TIIGER_online.lua')  
os.execute('./tg -s ./TIIGER.lua $@ --bot='..TIIGER_on[1])
end
...
os.execute('cd && rm -fr .telegram-cli')
os.execute('./tg -s ./TIIGER.lua $@ --bot='..tokenCk)
else
...
local data = JSON:decode(req)
if data.ok == true then
os.execute('./tg -s ./TIIGER.lua $@ --bot='..token)
else
