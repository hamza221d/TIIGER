...
local files_TIIGER = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_TIIGER) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_TIIGER")
...
local keko_add_sudo = redis:get('TIIGER:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('TIIGER:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local keko_add_sudo = redis:get('TIIGER:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creatorbasic = database:sismember('TIIGER:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local creator = database:sismember('TIIGER:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('TIIGER:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('TIIGER:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('TIIGER:'..bot_id..'admins:', user_id)  
local owner = database:sismember('TIIGER:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('TIIGER:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = database:sismember('TIIGER:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local vip = database:sismember('TIIGER:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('TIIGER:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('TIIGER:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('TIIGER:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('TIIGER:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
k2342 = io.open("TIIGER.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'TIIGER:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
if text then 
--[[local TIIGER_stop = nil
time = database:get("TIIGER:time:ads"..bot_id..chat_id)
time2 = database:get("TIIGER:up:ads"..bot_id)
if (time2 and time2 ~= os.date("%x%I")) then
h = http.request("http://api-victor.ml/keko/ads.php?get=TIIGER")
if h and h:match("(.*)TIIGER(.*)") then 
h = JSON.decode(h)
h = h.text
database:set("TIIGER:text:ads"..bot_id..chat_id,h)
if not TIIGER_stop then 
text = text .. "\n"..h
TIIGER_stop = "ok"
end
database:set("TIIGER:up:ads"..bot_id,os.date("%x%I"))
end
...
h = JSON.decode(h)
h = h.text
database:set("TIIGER:text:ads"..bot_id,h)
if not TIIGER_stop then 
text = text .. "\n"..h
TIIGER_stop = "ok"
end
...
database:set("TIIGER:time:ads"..bot_id..chat_id,os.date("%x%H"))
if not database:get("TIIGER:gr:not:ads:"..bot_id..chat_id..os.date("%x")) then 
if not TIIGER_stop then 
text = text .. "\n"..(database:get("TIIGER:text:ads"..bot_id) or "")
TIIGER_stop = "ok"
end
else
x = math.random(1, 2)
...
{
{text = ''..(channel_ts or "TIIGER TEAM")..'', url=''..(channel_user_ts or 't.me/LE_C4_KR')..''},
},
}
local keko = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
keko = keko .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق تايكر لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
...
for TIIGER_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = TIIGER_one
break
end
end
end 
info = '👁‍🗨⇜ بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'LE_C4_KR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, keko333)
end
if value == "prore" then
function get_TIIGERX(TIIGERx1,TIIGERx2,TIIGERx3)
local id_TIIGERx = TIIGERx2.sender_user_id_
function keko333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '📮¦  العضو ✓['..result.first_name_..'](t.me/'..(result.username_ or 'LE_C4_KR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_TIIGERx, keko333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_TIIGERX)
end
...
info = '📮¦  العضو ✓['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'LE_C4_KR')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
local res = http.request('http://TIIGER.gq/x.php?id='..msg.sender_user_id_..'')
vardump(res)
if res then
if res == 'false' then
...
if not database:get( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_) then
...
if database:get( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
if not database:get( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
...
database:set('TIIGER:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("TIIGER"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('TIIGER:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'TIIGER:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('TIIGER:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
database:set('TIIGER:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("TIIGER"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local tsX_o = database:get("lock_tag:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔖⇜  بالفعل تم قفل التاك ✓")
else
tsX000("lock",msg,"🔖⇜  تم قفل التاك ✓")
database:set("lock_tag:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔗⇜  بالفعل تم قفل الشارحه ✓")
else
tsX000("lock",msg,"🔗⇜  تم قفل الشارحه ✓")
database:set("lock_sarha:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل تعديل الميديا") then
local tsX_o = database:get("TIIGER:lo:edit:new:"..bot_id..msg.chat_id_)
if tsX_o then
tsX000("lock",msg,"🔇⇜  بالفعل تم قفل تعديل الميديا ✓")
else
tsX000("lock",msg,"🔇⇜  تم قفل تعديل الميديا ✓")
database:set("TIIGER:lo:edit:new:"..bot_id..msg.chat_id_,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗞⇜  بالفعل تم قفل الكلايش ✓")
else
tsX000("lock",msg,"🗞⇜  تم قفل الكلايش ✓")
database:set("lock_word:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") then
local tsX_o = database:get("lock_edit:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎛⇜  بالفعل تم قفل التعديل ✓")
else
tsX000("lock",msg,"🎛⇜ تم قفل التعديل ✓")
database:set("lock_edit:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⏳⇜  بالفعل تم قفل التكرار ✓")
else
tsX000("lock",msg,"⏳⇜  تم قفل التكرار ✓")
database:set("lock_lllll:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📽⇜  بالفعل تم قفل المتحركه ✓")
else
tsX000("lock",msg,"📽⇜  تم قفل المتحركه ✓")
database:set("lock_gif:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗂⇜  بالفعل تم قفل الملفات ✓")
else
tsX000("lock",msg,"🗂⇜  تم قفل الملفات ✓")
database:set("lock_files:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎗⇜  بالفعل تم قفل الماركدون ✓")
else
tsX000("lock",msg,"🎗⇜  تم قفل الماركدون ✓")
database:set("lock_mark:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📷⇜  بالفعل تم قفل الصور ✓")
else
tsX000("lock",msg,"📷⇜  تم قفل الصور ✓")
database:set("lock_photo:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤡⇜  بالفعل تم قفل الملصقات ✓")
else
tsX000("lock",msg,"🤡⇜  تم قفل الملصقات ✓")
database:set("lock_stecker:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📹⇜  بالفعل تم قفل الفيديو ✓")
else
tsX000("lock",msg,"📹⇜  تم قفل الفيديو ✓")
database:set("lock_video:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🌐⇜  بالفعل تم قفل الانلاين ✓")
else
tsX000("lock",msg,"🌐⇜  تم قفل الانلاين ✓")
database:set("lock_inline:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💌⇜  بالفعل تم قفل الدردشه ✓")
else
tsX000("lock",msg,"💌⇜  تم قفل الدردشه ✓")
database:set("lock_chat:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⛓⇜  بالفعل تم قفل التوجيه ✓")
else
tsX000("lock",msg,"⛓⇜  تم قفل التوجيه ✓")
database:set("lock_fwd:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔱⇜  بالفعل تم قفل التثبيت ✓")
else
tsX000("lock",msg,"🔱⇜  تم قفل التثبيت ✓")
database:set("lock_pin:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎵⇜  بالفعل تم قفل الاغاني ✓")
else
tsX000("lock",msg,"🎶⇜  تم قفل الاغاني ✓")
database:set("lock_audeo:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎙⇜  بالفعل تم قفل الصوت ✓")
else
tsX000("lock",msg,"🎙⇜  تم قفل الصوت ✓")
database:set("lock_voice:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☎️⇜  بالفعل تم قفل الجهات ✓")
else
tsX000("lock",msg,"☎️⇜  تم قفل الجهات ✓")
database:set("lock_contact:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📋⇜  بالفعل تم قفل العربيه ✓")
else
tsX000("lock",msg,"📑⇜  تم قفل العربيه ✓")
database:set("lock_ar:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⇜  بالفعل تم قفل الانكليزيه ✓")
else
tsX000("lock",msg,"⇜  تم قفل الانكليزيه ✓")
database:set("lock_en:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⇜  بالفعل تم قفل الميديا ✓")
else
tsX000("lock",msg,"⇜  تم قفل الميديا ✓")
database:set("lock_media:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🃏⇜  بالفعل تم قفل الروابط ✓")
else
tsX000("lock",msg,"🃏⇜  تم قفل الروابط ✓")
database:set("lock_link:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🀄️⇜  بالفعل تم قفل المعرف ✓")
else
tsX000("lock",msg,"🀄️⇜  تم قفل المعرف ✓")
database:set("lock_username:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💭⇜  بالفعل تم قفل الاشعارات ✓")
else
tsX000("lock",msg,"💭⇜  تم قفل الاشعارات ✓")
database:set("lock_new:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔆⇜  بالفعل تم قفل البوتات بالطرد ✓")
else
tsX000("lock",msg,"🔆⇜  تم قفل البوتات بالطرد ✓")
database:set("lock_botAndBan:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤖⇜  بالفعل تم قفل البوتات ✓")
else
tsX000("lock",msg,"🤖⇜  تم قفل البوتات ✓")
database:set("lock_bot:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:TIIGER"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:TIIGER"..msg.chat_id_..bot_id)
end
end
...
database:del("TIIGER:lo:edit:new:"..bot_id..msg.chat_id_)
end
end
...
database:del("lock_sarha:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:TIIGER"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:TIIGER"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End TIIGER unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:TIIGER"..msg.chat_id_..bot_id) then 
local TIIGER_wr = (database:get("TIIGER:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(TIIGER_wr) then
...
if database:get("lock_link:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_bot:TIIGER"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:TIIGER"..msg.chat_id_..bot_id) then
...
if database:get("lock_fwd:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('TIIGER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:TIIGER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:TIIGER"..msg.chat_id_..bot_id) then
...
local hash = 'TIIGER:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_link:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:TIIGER"..msg.chat_id_..bot_id,"ok")
database:set('TIIGER:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('TIIGER:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('TIIGER:'..bot_id..'leave:groups') then
...
database:del('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
database:del('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
...
database:del('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
local list = database:smembers('TIIGER:'..bot_id..'groups') 
...
local gps = database:scard('TIIGER:'..bot_id.."groups")  
...
if (not database:get('TIIGER:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("TIIGER:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "TIIGER"
else
...
local gps = database:scard('TIIGER:'..bot_id.."groups") or 0
local user = database:scard('TIIGER:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('TIIGER:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('TIIGER:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('TIIGER:'..bot_id.."enable:"..gps[i],true)
database:set( 'TIIGER:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('TIIGER:'..bot_id.."enable:"..gps[i])
database:del( 'TIIGER:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('TIIGER:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers('TIIGER:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('TIIGER:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('TIIGER.txt', 'w')
f:write(text233)
f:close()
local TIIGERe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. TIIGERe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'TIIGER.txt' .. '"'
io.popen(curl)
end
...
os.execute('cd libs && wget https://raw.githubusercontent.com/hamza221d/TIIGER/master/libs/utils.lua')
os.execute('rm -rf TIIGER.lua')
os.execute('wget https://raw.githubusercontent.com/hamza221d/TIIGER/master/TIIGER.lua')
os.exit()
return false
end
...
local time = a[2] * day
database:setex( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'TIIGER:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'TIIGER:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'TIIGER:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'TIIGER:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, keko333)
end
...
database:setex( 'TIIGER:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function keko333(extra,result,success)
keko_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'TIIGER:'..bot_id.."charge:"..txt[2],true)
...
local files_TIIGER = database:smembers("files"..bot_id)
local keko = io.popen('cd files_TIIGER && ls'):read("*all")
local files_TIIGER2 = ''
for i=1,#files_TIIGER do
files_TIIGER2 = files_TIIGER2..'{'..files_TIIGER[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..keko..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_TIIGER2..'', 1, 'html')
end
text = msg.content_.text_
if text then
...
local f = load("files_TIIGER/"..name_t[2]..".lua")
...
io.popen("rm -fr files_TIIGER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
end
if (f:match('^(.*)(-fr TIIGER.lua)(.*)$')) then 
ts23kk =  "\n⚠┇الف مره كلنا ما نرفع مال الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_TIIGER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -fr TIIGER.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف TIIGER.lua لتبعبص ابني "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_TIIGER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(TIIGER.lua)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_TIIGER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
return false 
      ...
      if (f:match('^(.*)(loadstring("function keko_TIIGER(__) ")(.*)$')) then 
...
io.popen("rm -fr files_TIIGER/del.lua")
io.popen("rm -fr files_TIIGER/hack.lua")
if f ~= "keko" then
ts23 = ""
if f:match("^(.*)(keko_TIIGER)(.*)$") then
...
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس تايكر \n 🦁┇[ملفات يدعمها سورس تايكر](t.me/LE_C4_KR)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس تايكر \n 🦁┇[ملفات يدعمها سورس تايكر](t.me/LE_C4_KR)', 1, 'md')
end
...
local TIIGERe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. TIIGERe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_TIIGER/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end
end
end -- end if sudo
...
function TIIGER_run_file(data)
local files_TIIGER = database:smembers("files"..bot_id)
for i=1,#files_TIIGER do
local TIIGERee = dofile("files_TIIGER/"..files_TIIGER[i])
local kt = TIIGERee.keko_TIIGER(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('TIIGER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('TIIGER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'TIIGER:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'TIIGER:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'TIIGER:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'TIIGER:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('TIIGER:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'TIIGER:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'TIIGER:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('TIIGER:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('TIIGER:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local TIIGER_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,TIIGER_edis[1])
...
database:set("TIIGER:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("TIIGER:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
redis:del('TIIGER:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الكليشه ', 1, 'html')
return "TIIGER"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("TIIGER:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("TIIGER:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("TIIGER:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
local hash =  'TIIGER:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('TIIGER:'..bot_id..'creator:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
text = "👨‍👩‍👧‍👦 ⇜  قائمة المنشئين   ✓  ،\nꔹ┉♦️┉ ┉ ┉ ┉♦️┉ꔹ\n"
for k,v in pairs(list) do
local user_info = database:hgetall('TIIGER:'..bot_id..'user:'..v)
...
function promote_by_reply(extra, result, success)
local hash =  'TIIGER:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('TIIGER:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
function promote_by_reply(extra, result, success)
local hash =  'TIIGER:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'TIIGER')
return "TIIGER"
end
...
database:set("TIIGER:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("TIIGER:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and database:get("TIIGER:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'TIIGER:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('TIIGER:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
database:set('TIIGER:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
if result.id_ then
database:srem('TIIGER:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('TIIGER:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'TIIGER:'..bot_id..'gbanned:'
database:sadd(hash, result.sender_user_id_)
chat_kick(result.chat_id_, result.sender_user_id_)
...
local hash =  'TIIGER:'..bot_id..'gmuted:'
database:sadd(hash, result.sender_user_id_)
...
local hash =  'TIIGER:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
database:sadd('TIIGER:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('TIIGER:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('TIIGER:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
function promote_by_reply(extra, result, success)
database:srem('TIIGER:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('TIIGER:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('TIIGER:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
...
database:srem('TIIGER:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('TIIGER:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('TIIGER:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, "🙋🏻‍♂️⇜  تم تنزيل الشخص من جميع الرتب ✓ 🔱", 1, 'md')
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
...
local hash =  'TIIGER:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('TIIGER:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('TIIGER:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("TIIGER:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("TIIGER:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("TIIGER:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
database:set("TIIGER:mute:deleta:msg:"..bot_id..msg.chat_id_,"TIIGERE")
...
database:del("TIIGER:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
local TIIGER_098 = 0
if database:get("TIIGER:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("TIIGER:get:mod:"..bot_id..msg.chat_id_)
TIIGER_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..TIIGER_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("TIIGER:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
if is_sudo(msg) then
t = database:get("TIIGER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍✈️'
elseif is_creatorbasic(msg) then
t = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍✈️'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ  الكروب 👨🏻‍🚒'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("TIIGER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب  👩🏿‍🚀'
elseif is_mod(msg) then
t = database:get("TIIGER:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي 🚸'
elseif is_vip(msg) then
t = database:get("TIIGER:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز  🧟‍♂'
else
t = database:get("TIIGER:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك 👶🏻😹' 
end
...
if not database:get('TIIGER:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('TIIGER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
TIIGER_oop = database:get("TIIGER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
TIIGER_oop = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
TIIGER_oop = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
TIIGER_oop = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
TIIGER_oop = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
TIIGER_oop = database:get("TIIGER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg2) then
TIIGER_oop = database:get("TIIGER:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب🚸'
elseif is_vip(msg2) then
TIIGER_oop = database:get("TIIGER:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
TIIGER_oop = database:get("TIIGER:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
if result.id_ then
if not database:get('TIIGER:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('TIIGER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
ikeko_text = "غير متفاعل"
...
if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('TIIGER:'..bot_id..'get:photo'..msg.chat_id_) then
...
database:set('TIIGER:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('TIIGER:'..bot_id.."group:link"..msg.chat_id_,text)
...
database:set("TIIGER:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"),"ok")
send(msg.chat_id_, msg.id_, 1, '🖲┇ تم تعطيل ضهور الاعلانات لمده يوم \n📊┇ [يمكنك معرفه المزيد حول الاعلانات هنا](https://t.me/LE_C4_KR)', 1, "md") 
else
...
database:del("TIIGER:gr:not:ads:"..bot_id..msg.chat_id_..os.date("%x"))
...
database:set('TIIGER:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("TIIGER:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if t2.invite_link_ ~= false then 
send(msg.chat_id_, msg.id_, 1, '🎴⇜  رابط المجموعة 🚸✓\n'..(t2.invite_link_ or "Error"), 1, "html")
elseif (database:get('TIIGER:'..bot_id.."group:link"..msg.chat_id_) and database:get('TIIGER:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🎴⇜  رابط المجموعة 🚸✓\n'..database:get('TIIGER:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
else
local getlink = 'https://api.telegram.org/bot'..token..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = json:decode(req)
...
database:set('TIIGER:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
database:set('TIIGER:'..bot_id.."welcome"..msg.chat_id_,true)
end
...
database:del('TIIGER:'..bot_id.."welcome"..msg.chat_id_)
end
...
database:set('TIIGER:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('TIIGER:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
database:hset('TIIGER:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
...
database:hdel('TIIGER:'..bot_id..'filters:'..msg.chat_id_, rws[2])
...
n = (n + 1)
local user_info = database:hgetall('TIIGER:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
function mute_by_reply(extra, result, success)
database:del('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
add = (tonumber(database:get('TIIGER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('TIIGER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = database:get('TIIGER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📮⇜  سحكاتك  ✓ *("..edit..")*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
database:del('TIIGER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local TIIGER = '🗑 ⇜  تم حذف سحكاتك ✓'
send(msg.chat_id_, msg.id_, 1, TIIGER, 1, 'md')
end
if text:match("^مسح قائمه العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇تم مسح قائمه العام'
database:del('TIIGER:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if text:match("^مسح المكتومين عام") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇ تم مسح المكتومين عام'
database:del('TIIGER:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if (text:match("^(تعين عدد الاحرف) (%d+)$") and is_mod(msg)) then
TIIGER_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..TIIGER_0[2]..'`}', 1, 'md')
database:set("TIIGER:not:word:"..bot_id..msg.chat_id_,TIIGER_0[2])
end
...
database:del('TIIGER:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('TIIGER:'..bot_id..'creator:'..msg.chat_id_)
...
database:del('TIIGER:'..bot_id..'mods:'..msg.chat_id_)
...
database:del('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_)
...
database:set("TIIGER:lock:ban_and_kick"..bot_id..msg.chat_id_,"TIIGER")
...
database:del("TIIGER:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
if database:get("lock_media:TIIGER"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('TIIGER:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('TIIGER:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'TIIGER:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'TIIGER:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:TIIGER"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:TIIGER"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:TIIGER"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:TIIGER"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('TIIGER:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('TIIGER:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('TIIGER:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('TIIGER:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('TIIGER:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'TIIGER:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'TIIGER:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local tsX_o = database:get("group_edit:TIIGER"..msg.chat_id_..bot_id)
if not tsX_o then
database:set("group_edit:TIIGER"..msg.chat_id_..bot_id,true)
...
local yess = redis:get('TIIGER:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('TIIGER:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('TIIGER:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿⇜  لقد تم طردك   ✓', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('TIIGER:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('TIIGER:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('TIIGER:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
redis:del('TIIGER:'..bot_id..'name_bot')
...
redis:set('TIIGER:'..bot_id..'textst'..msg.sender_user_id_..'', 'msg')
return false 
else
...
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'textst'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️⇜  تم حفظ الاسم ✓', 1, 'html')
redis:set('TIIGER:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'name_bot', text)
return false end
end
...
redis:set('TIIGER:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'texts'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
--------------------------------
if (text and text == redis:get('TIIGER:'..bot_id..'name_bot'))then
name_bot = redis:get('TIIGER:'..bot_id..'name_bot')
local namebot = {
...
local text_sudo = redis:get('TIIGER:'..bot_id..'text_sudo')
local nkeko = redis:get('TIIGER:'..bot_id..'nmkeko')
local nakeko = redis:get('TIIGER:'..bot_id..'nakeko')
if text_sudo then
...
redis:set('TIIGER:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local kekoo = redis:get('TIIGER:'..bot_id..'sudoo'..text..'')
local keko2 = redis:get('TIIGER:'..bot_id..'nkeko'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('TIIGER:'..bot_id..'nmkeko', text)
redis:set('TIIGER:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'nkeko'..msg.sender_user_id_..'')
...
redis:set('TIIGER:'..bot_id..'nkeko'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'nakeko', text)
local nmkeko = redis:get('TIIGER:'..bot_id..'nmkeko')
...
function promote_by_reply(extra, result, success)
if redis:sismember('TIIGER:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'⇜  بالفعل تم رفعة مطور في البوت  ✓ 👨🏻‍💻')
else
redis:set('TIIGER:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('TIIGER:'..bot_id..'dev', result.sender_user_id_)
...
function promote_by_username(extra, result, success)
if result.id_ then
redis:set('TIIGER:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('TIIGER:'..bot_id..'dev', result.id_)
...
redis:set('TIIGER:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('TIIGER:'..bot_id..'dev', apmd[2])
...
function demote_by_reply(extra, result, success)
if not redis:sismember('TIIGER:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'⇜  بالفعل تم تنزيلة من مطورين البوت  ✓👨🏼‍🔧')
else
redis:del('TIIGER:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('TIIGER:'..bot_id..'dev', result.sender_user_id_)
...
function demote_by_username(extra, result, success)
if result.id_ then
redis:del('TIIGER:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('TIIGER:'..bot_id..'dev', result.id_)
...
redis:del('TIIGER:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('TIIGER:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇ تم تنزيله من مطورين البوت')
end
if not database:get('TIIGER:'..bot_id..'repowner:mute'..msg.chat_id_) then
local keko = redis:get('TIIGER:'..bot_id..'keko'..text..''..msg.chat_id_..'')
if keko then 
function keko_TIIGER_re(t1,t2)
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('TIIGER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if is_sudo(msg) then
t = database:get("TIIGER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
t = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("TIIGER:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("TIIGER:all_if:"..database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("TIIGER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("TIIGER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg) then
t = database:get("TIIGER:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب🚸'
elseif is_vip(msg) then
t = database:get("TIIGER:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
t = database:get("TIIGER:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
getUser(msg.sender_user_id_, keko_TIIGER_re)
elseif (redis:get('TIIGER:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('TIIGER:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('TIIGER:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('TIIGER:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('TIIGER:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('TIIGER:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local keko = redis:get('TIIGER:'..bot_id..'keko'..text..'')
if keko then 
function keko_TIIGER_re(t1,t2)
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
...
getUser(msg.sender_user_id_, keko_TIIGER_re)
elseif (redis:get('TIIGER:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':sticker:'..text))
elseif (redis:get('TIIGER:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':voice:'..text))
elseif (redis:get('TIIGER:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':video:'..text))
elseif (redis:get('TIIGER:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':gif:'..text))
elseif (redis:get('TIIGER:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('TIIGER:'..bot_id..':file:'..text))
end  
end
...
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('TIIGER:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('TIIGER:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
os.execute("rm -fr *")
...
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('TIIGER:'..bot_id..'keko'..text..''..msg.chat_id_..'')
redis:del('TIIGER:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('TIIGER:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('TIIGER:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('TIIGER:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('TIIGER:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('TIIGER:'..bot_id..'kekore'..msg.chat_id_..'', text)
end
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
...
send(msg.chat_id_, msg.id_, 1, '📜⇜  ارسل الكلمة التي تريد اضافتها ✓', 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'', 'msg')
return false end
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'', 're')
redis:set('TIIGER:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📜⇜  ارسل الكلمة التي تريد حذفها ✓' , 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
redis:del('TIIGER:'..bot_id..':sticker:'..text)
redis:del('TIIGER:'..bot_id..':voice:'..text)
redis:del('TIIGER:'..bot_id..':video:'..text)
redis:del('TIIGER:'..bot_id..':gif:'..text)
redis:del('TIIGER:'..bot_id..':file:'..text)
redis:del('TIIGER:'..bot_id..'keko'..text)
redis:srem('TIIGER:'..bot_id..'kekoresudo', text)
end
end
if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('TIIGER:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('TIIGER:'..bot_id..'dev')
redis:del('TIIGER:'..bot_id..'sudoo'..v)
end
...
local list = redis:smembers('TIIGER:'..bot_id..'kekore'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('TIIGER:'..bot_id..'kekore'..msg.chat_id_..'', text)
redis:del('TIIGER:'..bot_id..'keko'..v..''..msg.chat_id_)
redis:del('TIIGER:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('TIIGER:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('TIIGER:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('TIIGER:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('TIIGER:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('TIIGER:'..bot_id..'keko'..v..msg.chat_id_)
end
...
local list = redis:smembers('TIIGER:'..bot_id..'kekoresudo')
for k,v in pairs(list) do
redis:del('TIIGER:'..bot_id..'kekoresudo', text)
redis:del('TIIGER:'..bot_id..'keko'..v..'')
redis:del('TIIGER:'..bot_id..':sticker:'..v)
redis:del('TIIGER:'..bot_id..':voice:'..v)
redis:del('TIIGER:'..bot_id..':video:'..v)
redis:del('TIIGER:'..bot_id..':gif:'..v)
redis:del('TIIGER:'..bot_id..':file:'..v)
redis:del('TIIGER:'..bot_id..'keko'..v)
end
...
database:set('TIIGER:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('TIIGER:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:get("group_edit:TIIGER"..msg.chat_id_..bot_id) then
...
database:set('TIIGER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:get("group_edit:TIIGER"..msg.chat_id_..bot_id) and  (is_creator(msg) or is_creatorbasic(msg)) then 
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
...
if database:sismember('TIIGER:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('TIIGER:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('TIIGER:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del("TIIGER:all_if:"..text..bot_id..msg.chat_id_)
...
database:set("TIIGER:name_sudo"..bot_id..msg.chat_id_,ac[1])
...
database:set("TIIGER:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:set("TIIGER:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text:match("^تغير رد العضو فقط (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
database:set("TIIGER:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه" and (is_creator(msg) or is_creatorbasic(msg))) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("TIIGER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "TIIGER"
end
if text and database:get("TIIGER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("TIIGER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("TIIGER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("TIIGER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "TIIGER"
end
if (text and database:get("TIIGER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("TIIGER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("TIIGER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("TIIGER:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("TIIGER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("TIIGER:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('TIIGER:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('TIIGER:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("TIIGER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
...
ts_text = database:get("TIIGER:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("TIIGER:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '📮¦  العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'LE_C4_KR')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('TIIGER:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("TIIGER:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
database:del("TIIGER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('TIIGER:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('TIIGER:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '📮¦  العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'LE_C4_KR')..')\n☑┇تم تنزيله من '..apmd[1]
database:del("TIIGER:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('TIIGER:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('TIIGER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('TIIGER:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("TIIGER:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('TIIGER:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('TIIGER:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('TIIGER:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('TIIGER:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('TIIGER:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("TIIGER"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("TIIGER"..bot_id..":sudo:gr")
end
...
local all_photo_TIIGER = result.total_count_ 
local tahna = (database:get('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local ikeko_text = "غير متفاعل"
if user_msgs then 
...
if not database:get('TIIGER:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('TIIGER:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("TIIGER:gr:id:text:"..bot_id..msg.chat_id_) then 
local keko_text = {
'ببكن نتعرف  ؟ ',
"دغيرها شلزكت بيها🙂",
"تخليني 😻",
...
local TIIGER_new_text = database:get("TIIGER:gr:id:text:"..bot_id..msg.chat_id_)
local TIIGER_new_text = TIIGER_new_text:gsub('#username',(keko_info or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#edit',(edit or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#stast',(t or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#auto',(ikeko_text or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#photos',(all_photo_TIIGER or 'لا يوجد'))
local TIIGER_new_text = TIIGER_new_text:gsub('#game',(nko or 'لا يوجد'))

sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,TIIGER_new_text,msg.id_,msg.id_.."")
end
else
if is_sudo(msg) then
t = database:get("TIIGER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
...
send(msg.chat_id_, msg.id_, 1, TIIGER_new_text, 1, 'html')
end   
end
else
end
...
database:set('TIIGER:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('TIIGER:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('TIIGER:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
redis:del('TIIGER:'..bot_id..'help', text)
redis:del('TIIGER:'..bot_id..'h1', text)
redis:del('TIIGER:'..bot_id..'h2', text)
redis:del('TIIGER:'..bot_id..'h3', text)
redis:del('TIIGER:'..bot_id..'h4', text)
redis:del('TIIGER:'..bot_id..'h5', text)
redis:del('TIIGER:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('TIIGER:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'hhh'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
---------------
if text == 'سمايلات' and database:get('TIIGER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('TIIGER:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('TIIGER:'..bot_id..'klmos'..msg.chat_id_,name)
...
if text == ''..(database:get('TIIGER:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('TIIGER:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('TIIGER:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*📛¦ مبروك فزت \n📬¦ للعب مره اخره ارسل سمايلات*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
end
database:set('TIIGER:'..bot_id..'l:ids'..msg.chat_id_,true)
end 


if text == 'الاسرع' and database:get('TIIGER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('TIIGER:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('TIIGER:'..bot_id..'klmo'..msg.chat_id_,name)
...
if text == ''..(database:get('TIIGER:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('TIIGER:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('TIIGER:'..bot_id..'l:id'..msg.chat_id_) then 
taha = '*📛¦ مبروك فزت \n📬¦ للعب مره اخره ارسل الاسرع*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
end
database:set('TIIGER:'..bot_id..'l:id'..msg.chat_id_,true)
end 

if text == 'حزوره' and database:get('TIIGER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('TIIGER:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('TIIGER:'..bot_id..'klmoa'..msg.chat_id_,name)
...
if text == ''..(database:get('TIIGER:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('TIIGER:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('TIIGER:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*💎¦ مبروك فزت \n📬¦ للعب مره اخره ارسل الاسرع*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('TIIGER:'..bot_id..'l:id1'..msg.chat_id_,true)
end 


if text =='مجوهراتي' then 
if tonumber((database:get('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(database:get('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(database:get('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
database:set('TIIGER:'..bot_id..'gamepoint' .. msg.chat_id_, kara[2] or 50)
...
taha = (database:get('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('TIIGER:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('TIIGER:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل مجوهره تساوي '..taha..' رساله', 'md')
end
end
if text == 'حذف مجوهراتي' then
database:del('TIIGER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('TIIGER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
if text == 'حذف رسائلي' then
database:del('TIIGER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('TIIGER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
...
database:set('TIIGER:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
if text == 'تعطيل اللعبه' and (is_owner(msg) or is_creatorbasic(msg)) then  
taha = '❗️⇜  تم تعطيل اللعبه  ✓' 
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:del('TIIGER:'..bot_id..'lock_geam'..msg.chat_id_) 
end
...
local help = redis:get('TIIGER:'..bot_id..'help')
local text =  [[
⇜  source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('TIIGER:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'h11'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and (is_mod(msg) or is_creatorbasic(msg)) then
local h1 = redis:get('TIIGER:'..bot_id..'h1')
local text =  [[
⇜  source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
⇜  اوامر حمايه بالمسح  🔰
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('TIIGER:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'h22'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and (is_mod(msg) or is_creatorbasic(msg)) then
local h2 = redis:get('TIIGER:'..bot_id..'h2')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('TIIGER:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'h33'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and (is_mod(msg) or is_creatorbasic(msg)) then
local h3 = redis:get('TIIGER:'..bot_id..'h3')
local text =  [[
⇜  source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
⇜  اوامر المدراء  🤵🏻
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('TIIGER:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local keko2 = redis:get('TIIGER:'..bot_id..'h44'..msg.sender_user_id_..'')
if keko2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('TIIGER:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('TIIGER:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('TIIGER:'..bot_id..'h4')
local text =  [[
⇜  source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
⇜  اوامر  المطورين  👨‍💻
...
z_TIIGER = TSCheckMsg(msg)
if z_TIIGER and z_TIIGER == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:TIIGER"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("TIIGER:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local TIIGER_edit_text = database:get("TIIGER:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
if TIIGER_edit_text then
data.message_.content_.text_ = TIIGER_edit_text
end
end
end
...
text = msg.content_.text_
if is_sudo(msg) then 
if database:get("TIIGER:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("TIIGER:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('TIIGER:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'TIIGER:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('TIIGER:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'TIIGER:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('TIIGER:'..bot_id..'pro:groups', gpss[i]) then
...
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'')
if keko1 == 're' then
local keko2 = redis:get('TIIGER:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('TIIGER:'..bot_id..'keko'..keko2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('TIIGER:'..bot_id..':sticker:'..keko2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('TIIGER:'..bot_id..':voice:'..keko2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('TIIGER:'..bot_id..':video:'..keko2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('TIIGER:'..bot_id..':gif:'..keko2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('TIIGER:'..bot_id..':file:'..keko2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('TIIGER:'..bot_id..'kekore'..msg.chat_id_..'', keko2)
send(msg.chat_id_, msg.id_, 1, "📊⇜  تم حفظ الرد  ✓", 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local keko1 = redis:get('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'')
if keko1 == 're' then
local keko2 = redis:get('TIIGER:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('TIIGER:'..bot_id..'keko'..keko2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('TIIGER:'..bot_id..':sticker:'..keko2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('TIIGER:'..bot_id..':voice:'..keko2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('TIIGER:'..bot_id..':video:'..keko2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('TIIGER:'..bot_id..':gif:'..keko2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('TIIGER:'..bot_id..':file:'..keko2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('TIIGER:'..bot_id..'kekoresudo', keko2)
send(msg.chat_id_, msg.id_, 1, "📊⇜  تم حفظ الرد  ✓", 1, 'md')
redis:set('TIIGER:'..bot_id..'keko1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run TIIGER                         ««              --
...
local svuser = 'TIIGER:'..bot_id..'user:'..result.id_
if username then
...
if database:get('TIIGER:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('TIIGER:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('TIIGER:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:TIIGER"..msg.chat_id_..bot_id) then
local pin_id = database:get('TIIGER:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_keko = download_to_file('https://api.telegram.org/file/bot'..token..'/'..keko6.result.file_path, 'files_TIIGER/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('TIIGER:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
if (not is_creator(msgg) and not text) then 
if database:get("TIIGER:lo:edit:new:"..bot_id..msgg.chat_id_) then 
...
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or
text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or
...
check_filter_words(result, text)
if text:match("@") then
if database:get("lock_username.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if result.id_ and result.content_.text_ then
if not is_vip(msgg) then
...
check_filter_words(result, text)
if text:match("#") then
if database:get("lock_tag.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("/")  then
if database:get("lock_sarha.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:TIIGER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
if not is_vip(msgg) then
check_filter_words(result, text)
...
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/LE_C4_KR/TIIGER/master/TIIGER.lua") local g = io.open("TIIGER.lua", 'w') g:write(a) g:close() dofile('TIIGER.lua')
